...
"""## **DEFINICIÓN DE LA FUNCIÓN PARA GRAFICAR MULTIPLES VECTORES LIBRES EN EL ESPACIO**
...
def plotvectorsl3D(*args):
    ...
    for V in args:
        ...
        cono = go.Cone(x=[V[1][0]-0.063*(V[1][0]-V[0][0])], y=[V[1][1]-0.063*(V[1][1]-V[0][1])], z=[V[1][2]-0.063*(V[1][2]-V[0][2])], u=[0.5*(V[1][0]-V[0][0])], v=[0.5*(V[1][1]-V[0][1])], w=[0.5*(V[1][2]-V[0][2])],sizemode="absolute",sizeref=0.5,anchor="cm",
               showscale=False,
               colorscale=paleta,
               hoverinfo = "skip",
               colorbar=dict(thickness=20, ticklen=4),
...
"""## **DEFINICIÓN DE LA FUNCIÓN PARA GRAFICAR MULTIPLES VECTORES LIBRES Y FIJOS EN EL ESPACIO**
La función tiene como sintaxis **plotvectors3D** permite graficar multiples vectores fijos y libres en el espacio.
"""
